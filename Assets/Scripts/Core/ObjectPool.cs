...
namespace Madasski.Core
{
    public class ObjectPool : MonoBehaviour
    {
        ...
        public static void AddObjectToPool(MonoBehaviour gameObject)
        {
            ...
            poolGameObject.transform.parent = Instance.transform;
        ...
        public static T GetObject<T>(T gameObjectToGet) where T : MonoBehaviour
        {
            if (_pools.ContainsKey(gameObjectToGet))
            {
                if (_pools[gameObjectToGet].Count > 0)
                {
                    return _pools[gameObjectToGet].Dequeue().GetComponent<T>();
                }
                else
                {
